...
> проверьте доступ командой из [шага 8](#шаг-8-проверка-сетевого-доступа).
...
| `WB` / `Ozon` / `Yandex: 403` | IP сервера заблокирован маркетплейсом | сменить локацию VPS (см. начало документа) |
...
2. Проверьте его прямо с сервера бота:
   ...
   ```bash
   curl -s -m 10 --socks5-hostname ЛОГИН:ПАРОЛЬ@IP:1080 \
        -o /dev/null -w "via proxy: %{http_code}\n" https://api.telegram.org
   ```

   Нужен `200`. Для HTTP-прокси вместо `--socks5-hostname` используйте `-x http://IP:3128`.
...
3. Пропишите прокси в `.env`:
   ...
   ```ini
   TELEGRAM_PROXY=socks5://ЛОГИН:ПАРОЛЬ@IP:1080
   ```

   Поддерживаются схемы `socks5://`, `socks4://` и `http://`; логин с паролем
   необязательны.
...
| «Маркетплейс закрыл доступ к карточке» на любую ссылку | IP сервера заблокирован WAF маркетплейса | выполнить проверку из [шага 8](#шаг-8-проверка-сетевого-доступа); помогает только смена локации VPS |

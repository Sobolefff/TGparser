...
scripts/check_access.py диагностика доступа к маркетплейсам
...
| `TELEGRAM_PROXY` | не задан | прокси **только** для вызовов Telegram API: `socks5://[user:pass@]host:port`, также `socks4://` и `http://`. Порт любой, но обязателен. Парсеры маркетплейсов всегда ходят напрямую |
...
| `PARSER_PROXY` | не задан | прокси **только** для запросов к маркетплейсам, независимо от `TELEGRAM_PROXY`. Нужен, когда WAF маркетплейса отвечает `403` на IP сервера |
...
Если бот отвечает «Маркетплейс закрыл доступ к карточке», запустите:

```bash
docker compose exec bot python scripts/check_access.py    # в Docker
uv run scripts/check_access.py                            # локально
```

Скрипт прогоняет реальные парсеры по эталонным ссылкам и показывает по каждому
маркетплейсу HTTP-статус и вердикт: заблокирован IP, изменилась схема API или всё в
порядке. Что делать с результатом — в [DEPLOY.md](DEPLOY.md#маркетплейсы-отвечают-403).

## Статус маркетплейсов

| Маркетплейс | Состояние |
| --- | --- |
| Wildberries | работает через `card.wb.ru` (нужен IP, не заблокированный WAF) |
| Ozon | работает через composer API (нужен IP, не заблокированный WAF) |
| Яндекс Маркет | **не работает**: Маркет отдаёт страницу без server-side данных (см. ниже) |

Яндекс Маркет перестал рендерить карточку на сервере: на запрос приходит `200` и ~1.4 МБ
JavaScript, без JSON-LD и без Open Graph-тегов — даже под User-Agent Googlebot. Парсить в
HTML нечего, поэтому `parsers/yandex.py` честно сообщает об этом пользователю вместо того,
чтобы выдавать «товара нет в продаже». Чтобы вернуть поддержку, нужен внутренний resolver
API Маркета (куки + антибот-токены) либо headless-браузер.
